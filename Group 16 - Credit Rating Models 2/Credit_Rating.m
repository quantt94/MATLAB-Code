...
%  we need a mapping onto the integers 1 through 24.  We will work around
...
%    % error 'oobError'
%    b = TreeBagger(nTrees,X_train,Y,'oobpred','on','cat',6,'minleaf',leaf(ii));
...
% A perfect result would be nonzero counts occurring only on the
% main diagonal of the confusion matrix.
...
% company = X(length(X) - [1:2],:);

% actual_credit = dataset1.RTG_SP_LT_LC_ISSUER_CREDIT(length(X)-[1:2]);
starbuck = X(364,:);
