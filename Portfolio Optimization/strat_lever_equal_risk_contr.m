% LEVERAGED EQUAL RISK CONTRIBUTION (ERC PORTFOLIO)
...
function [x, cash] = strat_lever_equal_risk_contr(curr_positions, curr_cash, mu, Q, cur_prices)
% number of stocks
n = 20;
curr_prices = cur_prices';

% short risk free asset for to take a long position of 100% more than the
% original position --> short a whole 100% portfolio value with risk-free
% rate
...
%total cost including borrowing cost at risk free rate
totalcost = sum(cost) + short_position*(exp(r_rf*(1/6))-1);
...
while cash < 0;
    ...
    cost = x_change.*curr_prices;
    transcost = sum(abs(cost).*0.005);
    totalcost = sum(cost) + (port_value/2)*(exp(r_rf/6)-1);
    cash = curr_cash - totalcost - transcost;
